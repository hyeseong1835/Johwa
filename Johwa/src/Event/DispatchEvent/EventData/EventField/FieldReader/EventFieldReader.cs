...
public abstract class EventFieldReader
{
    ...
    // 필드
    static Dictionary<Type, EventFieldReader> instanceDictionary = new ();
    ...
    static EventFieldReader GetOrCreateInstance(Type type)
    {
        if (instanceDictionary.TryGetValue(type, out EventFieldReader? instance))
        {
            return instance;
        }
        else
        {
            instance = (EventFieldReader?)Activator.CreateInstance(type);
            if (instance == null)
            {
                throw new JohwaException($"EventFieldReader ({type})를 생성할 수 없습니다.",
